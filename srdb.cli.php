...
ARGS
  ...
  --exclude-tables
    If set excludes the specified tables, comma separate for
    multiple values.
  -i, --include-cols
	...
	public function log( $type = '' ) {
		...
		switch( $type ) {
			case 'error':
				...
				break;
			case 'search_replace_table_excluded':
				list( $table ) = $args;
				$output .= "{$table}: excluded";
				break;
			case 'search_replace_table_start':
				list( $table, $search, $replace ) = $args;
				$output .= "{$table}: replacing {$search} with {$replace}";
				break;
            case 'search_replace_update_sql':
				list( $table, $sql ) = $args;
				$output .= "{$table}: SQL --> {$sql}";
				break;
